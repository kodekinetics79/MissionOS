...
function stationTarget(station: AnyRecord) {
  return Math.max(4, Number(station.staffingGap ?? 0) > 0 ? 8 - Number(station.staffingGap ?? 0) : 6);
}

function currentLeaveCount(records: AnyRecord[], personnelId: string) {
...
function getPersonnelLabel(personnel: AnyRecord) {
  return personnel.fullName ?? personnel.name ?? `${personnel.firstName ?? ''} ${personnel.lastName ?? ''}`.trim() ?? 'Personnel';
}

function getMinimumStaffingRule(station: AnyRecord) {
...
async function writeAudit(tenantId: string, userId: string | null | undefined, action: string, entityName: string, entityId?: string, before?: unknown, after?: unknown) {
...
async function writeNotification(tenantId: string, title: string, message: string, notificationType: string = 'staffing') {
...
async function getContext(tenantId: string) {
  const [
    ...
    prisma.personnel.findMany({ where: { tenantId }, include: { certifications: true, overtimeRecords: true, availability: true } }),
    prisma.shiftAssignment.findMany({ where: { tenantId }, include: { personnel: true, station: true, shift: true } }),
    prisma.openShift.findMany({ where: { tenantId }, include: { shift: true }, orderBy: { priority: 'desc' } }),
    prisma.staffingRule.findMany({ where: { tenantId } }),
    prisma.leaveRequest.findMany({ where: { tenantId } }),
    prisma.overtimeRecord.findMany({ where: { tenantId }, include: { personnel: true } }),
    prisma.availabilityRecord.findMany({ where: { tenantId }, include: { personnel: true } }),
    prisma.certification.findMany({ where: { tenantId } }),
    prisma.aiInsight.findMany({ where: { tenantId, OR: [{ category: { contains: 'Staffing' } }, { title: { contains: 'staffing' } }, { summary: { contains: 'staffing' } }] }, orderBy: { createdAt: 'desc' }, take: 10 }),
    prisma.notification.findMany({ where: { tenantId, OR: [{ notificationType: { contains: 'staffing' } }, { title: { contains: 'staffing' } }, { message: { contains: 'staffing' } }] }, orderBy: { createdAt: 'desc' }, take: 15 }),
  ]);
  ...
  const overtimeByPersonnel = new Map<string, number>();
  ...
  const availabilityByPersonnel = new Map<string, number>();
...
function stationCoverageRow(station: AnyRecord, ctx: Awaited<ReturnType<typeof getContext>>) {
  ...
  const frontline = apparatusInService(station).map((unit: AnyRecord) => unit.unitNumber);
...
function recommendedBackfillPool(ctx: Awaited<ReturnType<typeof getContext>>) {
...
function recommendationRows(ctx: Awaited<ReturnType<typeof getContext>>) {
...
function shiftFillRows(ctx: Awaited<ReturnType<typeof getContext>>) {
...
function tradeRows(ctx: Awaited<ReturnType<typeof getContext>>) {
  const recommendations = recommendationRows(ctx);
...
function overtimeRows(ctx: Awaited<ReturnType<typeof getContext>>) {
  return ctx.personnel
    .map((person) => {
...
function apparatusInService(station: AnyRecord) {
  return (station.apparatus ?? []).filter((unit: AnyRecord) => !['OUT_OF_SERVICE', 'RETIRED', 'Out of Service', 'Retired'].includes(String(unit.status ?? '')));
}
...
function frontlineMinimum(station: AnyRecord): { min: number; units: Array<[string, number]> } {
...
function minimumStaffingForStation(station: AnyRecord) {
  return frontlineMinimum(station).min;
}

function platoonForIndex(index: number) {
...
function rosterForDay(ctx: Awaited<ReturnType<typeof getContext>>, dayOffset: number) {
...
function callbackQueueRows(ctx: Awaited<ReturnType<typeof getContext>>) {
  return ctx.personnel
...
function forecastSeries(ctx: Awaited<ReturnType<typeof getContext>>, days: number) {
...
async function staffingDecisionAction(tenantId: string, userId: string | undefined, id: string, action: string) {
  const auditEntry = {
...
export const staffingService = {
  ...
  async getBoard(tenantId: string, page = 1, take = 50, filters: AnyRecord = {}) {
    const ctx = await getContext(tenantId);
    let rows = ctx.stations.map((station) => stationCoverageRow(station, ctx));
    if (filters.stationId) rows = rows.filter((row) => row.station.id === filters.stationId);
    if (filters.riskLevel) rows = rows.filter((row) => normalizeStatus(row.riskLevel) === normalizeStatus(filters.riskLevel));
    if (filters.status) rows = rows.filter((row) => normalizeStatus(row.staffingStatus) === normalizeStatus(filters.status));
    if (filters.search) {
  ...
  async getGaps(tenantId: string, page = 1, take = 50) {
    const ctx = await getContext(tenantId);
  ...
  async getRecommendations(tenantId: string, page = 1, take = 50) {
    const ctx = await getContext(tenantId);
    const recommendations = recommendationRows(ctx);
  ...
  async getShiftFill(tenantId: string, page = 1, take = 50) {
    const ctx = await getContext(tenantId);
  ...
  async getTrades(tenantId: string, page = 1, take = 50) {
    const ctx = await getContext(tenantId);
  ...
  async getOvertime(tenantId: string, page = 1, take = 50) {
    const ctx = await getContext(tenantId);
  ...
  async getAuditLog(tenantId: string, page = 1, take = 50) {
  ...
  async getRoster(tenantId: string, dayOffset = 0) {
    const ctx = await getContext(tenantId);
    return rosterForDay(ctx, dayOffset);
  },

  async getPlanner(tenantId: string, days = 5) {
    const ctx = await getContext(tenantId);
  ...
  async getForecast(tenantId: string, days = 7) {
    const ctx = await getContext(tenantId);
  ...
  async getKpis(tenantId: string) {
    ...
    const tradeApproval = clamp(Math.round((trades.filter((t) => normalizeStatus((t as AnyRecord).status) === 'approved').length / Math.max(1, trades.length)) * 100));
    return {
      coverage,
      minCompliance,
      fillRate,
  ...
  async getAppraisals(tenantId: string, page = 1, take = 50) {
    const ctx = await getContext(tenantId);
  ...
  async actOnRecommendation(tenantId: string, userId: string | undefined, id: string, payload: AnyRecord = {}) {
