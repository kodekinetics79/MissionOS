...
router.post('/login', asyncHandler(async (req, res) => {
  const body = z.object({ email: z.string().email(), password: z.string().min(1), totp: z.string().optional() }).parse(req.body);
...
router.get('/mfa/status', authRequired, asyncHandler(async (req, res) => ok(res, await mfaService.status(req.user!.tenantId, req.user!.userId), 'MFA status')));
router.post('/mfa/setup', authRequired, asyncHandler(async (req, res) => ok(res, await mfaService.setup(req.user!.tenantId, req.user!.userId), 'MFA setup started')));
