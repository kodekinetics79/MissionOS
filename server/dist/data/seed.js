...
const historicalIso = (daysAgo) => new Date(now - daysAgo * 86400000).toISOString();
...
const users = [
    ...
    ...user,
    tenantId,
    passwordHash: bcryptjs_1.default.hashSync('MissionOS2026!', 10),
...
const incidents = [...fixedIncidents, ...generatedIncidents].map((incident) => ({
    ...incident,
    tenantId,
...
const incidentAttachments = incidents.flatMap((incident, index) => (index % 2 === 0
    ? [{
...
const integrationLogs = integrationSeed.flatMap((descriptor, systemIndex) => Array.from({ length: 22 }).map((_, index) => {
    const seq = systemIndex * 22 + index;
    let status = LOG_STATUSES[seq % LOG_STATUSES.length];
    if (descriptor.status === 'Connected' && status === 'Failed' && index % 2 === 0)
        status = 'Success';
...
const INTEGRATION_ERROR_TEMPLATES = [
...
const webhookSubscriptions = integrationSeed.slice(0, 6).flatMap((descriptor, systemIndex) => Array.from({ length: 2 }).map((_, index) => ({
...
const integrationHealthSnapshots = integrationSeed.flatMap((descriptor) => Array.from({ length: 12 }).map((_, dayIndex) => {
...
while (aiInsightEvidence.length < 260) {
    ...
    aiInsightEvidence.push({
        ...
        sourceModule: insight.sourceModulesJson[0] ?? 'Analytics',
...
const dataQualityChecks = Array.from({ length: 20 }, (_, index) => {
    ...
    const counts = [
        ...
        personnelCertifications.filter((cert) => ['Expired', 'Expiring Soon'].includes(String(cert.status ?? ''))).length,
...
const analyticsSnapshots = [
    ...stations.flatMap((station, index) => ['Incidents', 'Training', 'Staffing', 'Personnel', 'Assets', 'Prevention', 'Integrations'].map((module, moduleIndex) => {
