...
exports.trainingReadinessImpactService = exports.trainingAttendanceService = exports.certificationComplianceService = exports.traineeRecommendationService = exports.trainerRecommendationService = exports.courseRecommendationService = exports.trainingNeedAssessmentService = void 0;
exports.listTrainingNeeds = listTrainingNeeds;
exports.generateTrainingNeeds = generateTrainingNeeds;
exports.getTrainingNeed = getTrainingNeed;
exports.dismissTrainingNeed = dismissTrainingNeed;
exports.resolveTrainingNeed = resolveTrainingNeed;
...
exports.getCertificationCompliance = getCertificationCompliance;
exports.getExpiringCertifications = getExpiringCertifications;
exports.createRenewal = createRenewal;
exports.getTrainingCommandCenter = getTrainingCommandCenter;
exports.getTrainerRecommendations = getTrainerRecommendations;
exports.getTraineeRecommendations = getTraineeRecommendations;
exports.getCourseRecommendations = getCourseRecommendations;
exports.getReadinessImpact = getReadinessImpact;
exports.getPersonnelTrainingProfile = getPersonnelTrainingProfile;
...
async function loadContext(tenantId) {
    const [personnel, stations, certifications, courses, sessions, assignments, attendance, outcomes, needs, reviews, incidents, incidentPersonnel, personnelAssignments, apparatus, inspections, properties, assets,] = await Promise.all([
        ...
        prisma_js_1.prisma.course.findMany({ where: { tenantId } }),
        prisma_js_1.prisma.courseSession.findMany({ where: { tenantId } }),
        ...
        prisma_js_1.prisma.trainingOutcome.findMany({ where: { tenantId } }),
        prisma_js_1.prisma.trainingNeedAssessment.findMany({ where: { tenantId } }),
        prisma_js_1.prisma.personnelPerformanceReview.findMany({ where: { tenantId } }),
        prisma_js_1.prisma.incident.findMany({ where: { tenantId } }),
        prisma_js_1.prisma.incidentPersonnel.findMany({ where: { tenantId } }),
        prisma_js_1.prisma.personnelAssignment.findMany({ where: { tenantId, isCurrent: true } }),
    ...
    const certificationLookup = new Map();
    ...
    const personnelCertifications = normalizeRows(await prisma_js_1.prisma.personnelCertification.findMany({ where: { tenantId } }));
    const certsByPersonnel = new Map();
    ...
    const reviewsByPersonnel = new Map();
    ...
    const incidentCountsByPersonnel = new Map();
    ...
    const incidentCountsByType = new Map();
    ...
    return {
        ...
        attendance,
        certificationLookup,
        courseCategories: await prisma_js_1.prisma.courseCategory.findMany({ where: { tenantId } }),
        certsByPersonnel,
        courses,
        instructorProfiles: await prisma_js_1.prisma.instructorProfile.findMany({ where: { tenantId } }),
        renewals: await prisma_js_1.prisma.certificationRenewal.findMany({ where: { tenantId } }),
        incidents,
        ...
        notifications: await prisma_js_1.prisma.notification.findMany({ where: { tenantId } }),
        aiInsights: await prisma_js_1.prisma.aiInsight.findMany({ where: { tenantId } }),
    };
}
function hasCertification(personCerts, requirement, certificationLookup) {
...
function activeAssignmentCount(assignmentsByPersonnel, personnelId) {
...
function buildTrainerScore(person, course, need, context) {
    const personCerts = context.certsByPersonnel.get(person.id) ?? [];
    const requiredCerts = (course.requiredCertifications ?? []).filter(Boolean);
    const certificationMatch = requiredCerts.length ? requiredCerts.filter((required) => hasCertification(personCerts, required, context.certificationLookup)).length / requiredCerts.length : 0.7;
    const relevantIncidents = context.incidents.filter((incident) => {
        ...
        return (course.relatedIncidentTypes ?? []).some((type) => incidentType.includes(String(type).toLowerCase()));
    });
    const subjectExperience = clamp01(relevantIncidents.length / 8 + ((context.incidentCountsByPersonnel.get(person.id) ?? 0) / 30));
    const instructorHistory = clamp01(((context.assignmentsByPersonnel.get(person.id) ?? []).filter((assignment) => statusCode(assignment.status) === 'COMPLETED').length / 8) + ((context.outcomes.filter((outcome) => outcome.personnelId === person.id && outcome.passed).length) / 8));
    ...
    const profileBoost = instructorProfile ? clamp01((Number(instructorProfile.teachingHistory ?? 0) / 10) + (instructorProfile.specialties?.some((item) => (course.category ?? '').toLowerCase().includes(String(item).toLowerCase())) ? 0.2 : 0)) : 0;
    ...
    const score = certificationMatch * 25 +
...
function buildTraineeScore(person, course, need, context) {
    const personCerts = context.certsByPersonnel.get(person.id) ?? [];
    const requiredCerts = (course.requiredCertifications ?? []).filter(Boolean);
    const missingRequired = requiredCerts.filter((required) => !hasCertification(personCerts, required, context.certificationLookup));
    ...
    const roleRequirement = clamp01((course.requiredForRoles ?? []).some((role) => String(role).toLowerCase() === String(person.role ?? person.rank ?? '').toLowerCase()) ? 1 : 0.25);
    ...
    const score = certificationGapScore * 25 +
...
function deriveNeedCourse(need, courses) {
...
async function listTrainingNeeds(tenantId, page = 1, take = 50, status, stationId) {
    ...
    const [items, total] = await Promise.all([
        prisma_js_1.prisma.trainingNeedAssessment.findMany({ where, take, skip: resolveSkip(page, take), orderBy: [{ priority: 'asc' }, { severity: 'desc' }, { updatedAt: 'desc' }] }),
        prisma_js_1.prisma.trainingNeedAssessment.count({ where }),
    ]);
    return { items, page, take, total };
}
async function generateTrainingNeeds(tenantId, userId) {
    const context = await loadContext(tenantId);
    const needs = [];
    const expiringCertGroups = new Map();
    ...
    for (const [incidentType, count] of incidentTrendTypes.slice(0, 2)) {
        ...
        const course = context.courses.find((item) => (item.relatedIncidentTypes ?? []).some((related) => lowerType.includes(String(related).toLowerCase())));
        if (!course)
            continue;
        needs.push({
    ...
    for (const need of orderedNeeds) {
        const existing = await prisma_js_1.prisma.trainingNeedAssessment.findFirst({
            where: {
                tenantId,
                title: need.title,
        ...
        const record = existing ? await prisma_js_1.prisma.trainingNeedAssessment.update({ where: { id: existing.id }, data }) : await prisma_js_1.prisma.trainingNeedAssessment.create({ data: { id: (0, node_crypto_1.randomUUID)(), ...data } });
    ...
    if (userId) {
        ...
        await Promise.all(savedNeeds.slice(0, 5).map((need) => prisma_js_1.prisma.notification.create({
            data: {
                tenantId,
                userId: userId ?? null,
...
async function getTrainingNeed(tenantId, needId) {
    return prisma_js_1.prisma.trainingNeedAssessment.findFirst({ where: { tenantId, id: needId } });
}
async function dismissTrainingNeed(tenantId, needId, userId) {
    const need = await prisma_js_1.prisma.trainingNeedAssessment.update({ where: { id: needId }, data: { status: 'Dismissed' } });
    if (userId) {
        await prisma_js_1.prisma.auditLog.create({ data: { tenantId, userId, action: `Dismissed training need ${need.title}`, entityName: 'TrainingNeedAssessment', entityId: needId } });
    }
    return need;
}
async function resolveTrainingNeed(tenantId, needId, userId) {
    const need = await prisma_js_1.prisma.trainingNeedAssessment.update({ where: { id: needId }, data: { status: 'Resolved' } });
    if (userId) {
        await prisma_js_1.prisma.auditLog.create({ data: { tenantId, userId, action: `Resolved training need ${need.title}`, entityName: 'TrainingNeedAssessment', entityId: needId } });
    }
    return need;
}
async function listCourses(tenantId, page = 1, take = 50, filters = {}) {
    const where = { tenantId };
    if (filters.category)
    ...
    const [items, total] = await Promise.all([
        prisma_js_1.prisma.course.findMany({ where, take, skip: resolveSkip(page, take), orderBy: { title: 'asc' } }),
        prisma_js_1.prisma.course.count({ where }),
    ]);
    const filtered = filters.certification ? items.filter((course) => (course.requiredCertifications ?? []).some((required) => required.toLowerCase().includes(filters.certification.toLowerCase()))) : items;
    return { items: filtered, page, take, total: filters.certification ? filtered.length : total };
}
async function getCourse(tenantId, courseId) {
    return prisma_js_1.prisma.course.findFirst({ where: { tenantId, id: courseId } });
}
async function createCourse(tenantId, payload, userId) {
    const course = await prisma_js_1.prisma.course.create({ data: { tenantId, ...payload } });
    if (userId)
        await prisma_js_1.prisma.auditLog.create({ data: { tenantId, userId, action: `Created course ${course.title}`, entityName: 'Course', entityId: course.id } });
    return course;
}
async function updateCourse(tenantId, courseId, payload, userId) {
    const course = await prisma_js_1.prisma.course.update({ where: { id: courseId }, data: payload });
    if (userId)
        await prisma_js_1.prisma.auditLog.create({ data: { tenantId, userId, action: `Updated course ${course.title}`, entityName: 'Course', entityId: course.id } });
    return course;
}
async function listSessions(tenantId, page = 1, take = 50, filters = {}) {
    const where = { tenantId };
    if (filters.courseId)
    ...
    const [items, total] = await Promise.all([
        prisma_js_1.prisma.courseSession.findMany({ where, take, skip: resolveSkip(page, take), orderBy: { startDateTime: 'asc' } }),
        prisma_js_1.prisma.courseSession.count({ where }),
    ]);
    return { items, page, take, total };
}
async function getSession(tenantId, sessionId) {
    return prisma_js_1.prisma.courseSession.findFirst({ where: { tenantId, id: sessionId } });
}
async function createSession(tenantId, payload, userId) {
    const session = await prisma_js_1.prisma.courseSession.create({ data: { tenantId, ...payload } });
    if (userId)
        await prisma_js_1.prisma.auditLog.create({ data: { tenantId, userId, action: `Scheduled session ${session.id}`, entityName: 'CourseSession', entityId: session.id } });
    return session;
}
async function updateSession(tenantId, sessionId, payload, userId) {
    const session = await prisma_js_1.prisma.courseSession.update({ where: { id: sessionId }, data: payload });
    if (userId)
        await prisma_js_1.prisma.auditLog.create({ data: { tenantId, userId, action: `Updated session ${session.id}`, entityName: 'CourseSession', entityId: session.id } });
    return session;
}
async function listAssignments(tenantId, page = 1, take = 50, filters = {}) {
    const where = { tenantId };
    ...
    if (filters.status)
        where.status = filters.status;
    const [items, total] = await Promise.all([
        prisma_js_1.prisma.trainingAssignment.findMany({ where, take, skip: resolveSkip(page, take), orderBy: { createdAt: 'desc' } }),
        prisma_js_1.prisma.trainingAssignment.count({ where }),
    ]);
    return { items, page, take, total };
}
async function createAssignment(tenantId, payload, userId) {
    const assignment = await prisma_js_1.prisma.trainingAssignment.create({ data: { tenantId, ...payload } });
    if (userId)
        await prisma_js_1.prisma.auditLog.create({ data: { tenantId, userId, action: `Assigned ${assignment.courseId} to ${assignment.personnelId}`, entityName: 'TrainingAssignment', entityId: assignment.id } });
    return assignment;
}
async function bulkCreateAssignments(tenantId, payload, userId) {
    const rows = Array.isArray(payload.assignments) ? payload.assignments : [];
    const created = [];
    for (const assignment of rows) {
        created.push(await prisma_js_1.prisma.trainingAssignment.create({ data: { tenantId, ...assignment } }));
    }
...
async function listAttendance(tenantId, sessionId) {
    return prisma_js_1.prisma.trainingAttendance.findMany({ where: { tenantId, sessionId }, orderBy: { createdAt: 'desc' } });
}
async function recordAttendance(tenantId, sessionId, payload, userId) {
    const existing = await prisma_js_1.prisma.trainingAttendance.findFirst({ where: { tenantId, sessionId, personnelId: payload.personnelId } });
    const attendance = existing
        ? await prisma_js_1.prisma.trainingAttendance.update({ where: { id: existing.id }, data: { ...payload, sessionId } })
        : await prisma_js_1.prisma.trainingAttendance.create({ data: { tenantId, sessionId, ...payload } });
    if (userId) {
        await prisma_js_1.prisma.auditLog.create({
            data: {
                tenantId,
    ...
    const assignment = await prisma_js_1.prisma.trainingAssignment.findFirst({ where: { tenantId, sessionId, personnelId: payload.personnelId } });
    if (assignment && ['COMPLETED', 'ATTENDED', 'PASSED'].includes(statusCode(payload.attendanceStatus))) {
        await prisma_js_1.prisma.trainingAssignment.update({ where: { id: assignment.id }, data: { status: 'Completed', completedAt: payload.checkOutTime ?? new Date().toISOString(), score: payload.participationScore ?? assignment.score } });
        await prisma_js_1.prisma.trainingOutcome.create({
            data: {
                tenantId,
...
async function getCertificationCompliance(tenantId) {
    ...
    const byCertification = certifications.map((certification) => {
        const relevant = personnelCertifications.filter((entry) => String(entry.certificationId) === String(certification.id));
        const active = relevant.filter((entry) => !entry.expiryDate || daysUntil(entry.expiryDate) > 0);
        const expiring = relevant.filter((entry) => daysUntil(entry.expiryDate) <= 90 && daysUntil(entry.expiryDate) > 0).length;
        const expired = relevant.filter((entry) => daysUntil(entry.expiryDate) <= 0).length;
        return {
            certification,
    ...
    const byStation = stations.map((station) => {
        const stationPersonnel = personnel.filter((entry) => entry.stationId === station.id);
        const criticalCerts = personnelCertifications.filter((entry) => stationPersonnel.some((person) => person.id === entry.personnelId) && daysUntil(entry.expiryDate) <= 60);
        return {
            station,
...
async function getExpiringCertifications(tenantId) {
...
async function createRenewal(tenantId, payload, userId) {
    const certification = await prisma_js_1.prisma.personnelCertification.create({
        data: {
            tenantId,
            personnelId: payload.personnelId,
    ...
    await prisma_js_1.prisma.certificationRenewal.create({
        data: {
            tenantId,
            personnelId: payload.personnelId,
            certificationId: payload.certificationId,
    ...
    if (userId)
        await prisma_js_1.prisma.auditLog.create({ data: { tenantId, userId, action: `Renewed certification ${payload.certificationId} for ${payload.personnelId}`, entityName: 'PersonnelCertification', entityId: certification.id } });
    return certification;
}
async function getTrainingCommandCenter(tenantId) {
    const [stats, needs, sessions, assignments, compliance, readiness, courseRecommendations, trainerRecommendations, traineeRecommendations, expiringCertifications, personnel, stations, aiInsights, notifications] = await Promise.all([
        ...
        prisma_js_1.prisma.trainingNeedAssessment.findMany({ where: { tenantId }, orderBy: [{ priority: 'asc' }, { updatedAt: 'desc' }], take: 20 }),
        prisma_js_1.prisma.courseSession.findMany({ where: { tenantId }, orderBy: { startDateTime: 'asc' }, take: 20 }),
        prisma_js_1.prisma.trainingAssignment.findMany({ where: { tenantId }, orderBy: { createdAt: 'desc' }, take: 50 }),
        ...
        prisma_js_1.prisma.personnel.findMany({ where: { tenantId } }),
        prisma_js_1.prisma.station.findMany({ where: { tenantId } }),
        prisma_js_1.prisma.aiInsight.findMany({ where: { tenantId, status: 'Open' } }),
    ...
    const complianceRate = compliance.byCertification.length
        ? Math.round(compliance.byCertification.reduce((total, item) => total + Number(item.complianceRate ?? 0), 0) / compliance.byCertification.length)
        : 0;
    const expiredCertifications = expiringCertifications.filter((item) => daysUntil(item.expiryDate ?? item.expiresAt) <= 0).length;
    const overdueAssignments = assignments.filter((assignment) => !['Completed', 'Waived'].includes(statusCode(assignment.status))).length;
    const upcomingSessions = sessions.filter((session) => daysUntil(session.startDateTime) >= 0).length;
    const stationTrainingRisk = stations.filter((station) => Number(station.readinessScore ?? 0) < 85).length;
    ...
    return {
        ...
        highPriorityNeeds: needs.filter((need) => Number(String(need.priority ?? '9')) <= 2).slice(0, 5),
        upcomingSessions,
        overdueAssignments,
        stationTrainingRisk,
        recommendedCourses,
...
async function getTrainerRecommendations(tenantId, needId, courseId) {
    const context = await loadContext(tenantId);
...
async function getTraineeRecommendations(tenantId, needId, courseId) {
    const context = await loadContext(tenantId);
    const needs = needId ? context.needs.filter((need) => need.id === needId) : context.needs;
    const results = [];
    for (const need of needs) {
...
async function getCourseRecommendations(tenantId) {
    const context = await loadContext(tenantId);
...
async function getReadinessImpact(tenantId) {
    const context = await loadContext(tenantId);
...
async function getPersonnelTrainingProfile(tenantId, personnelId) {
    const [personnel, assignments, certifications, reviews, outcomes, attendance] = await Promise.all([
        prisma_js_1.prisma.personnel.findFirst({ where: { tenantId, id: personnelId } }),
        prisma_js_1.prisma.trainingAssignment.findMany({ where: { tenantId, personnelId } }),
        prisma_js_1.prisma.personnelCertification.findMany({ where: { tenantId, personnelId } }),
        prisma_js_1.prisma.personnelPerformanceReview.findMany({ where: { tenantId, personnelId } }),
        prisma_js_1.prisma.trainingOutcome.findMany({ where: { tenantId, personnelId } }),
        prisma_js_1.prisma.trainingAttendance.findMany({ where: { tenantId, personnelId } }),
    ]);
    return {
        personnel,
        assignments,
        ...
        outcomes,
        attendance,
        readinessImpact: outcomes.reduce((sum, outcome) => sum + Number(outcome.readinessImpact ?? 0), 0),
        nextRecommendedTraining: assignments.find((assignment) => statusCode(assignment.status) !== 'COMPLETED') ?? null,
    };
}
exports.trainingNeedAssessmentService = {
    listTrainingNeeds,
...
exports.courseRecommendationService = { getCourseRecommendations };
exports.trainerRecommendationService = { getTrainerRecommendations };
exports.traineeRecommendationService = { getTraineeRecommendations };
exports.certificationComplianceService = { getCertificationCompliance, getExpiringCertifications, createRenewal, listCertifications };
exports.trainingAttendanceService = { listAttendance, recordAttendance };
exports.trainingReadinessImpactService = { getReadinessImpact };
