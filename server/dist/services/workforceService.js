...
const bandStatus = { Green: 'Healthy', Yellow: 'Warning', Red: 'At Risk', Critical: 'Critical' };
const KPI_SEED = [
...
function weightedComposite(values) {
...
async function getContext(tenantId) {
    const [personnel, stations] = await Promise.all([
        ...
        prisma_js_1.prisma.station.findMany({ where: { tenantId }, include: { apparatus: true }, orderBy: { number: 'asc' } }),
    ]);
...
async function stationScorecards(tenantId) {
    const ctx = await getContext(tenantId);
    const board = (await staffingService_js_1.staffingService.getBoard(tenantId, 1, 100)).items;
    return ctx.stations.map((station, index) => {
...
async function platoonScorecards(tenantId) {
    const ctx = await getContext(tenantId);
...
async function personnelScorecards(tenantId) {
    const ctx = await getContext(tenantId);
    return ctx.personnel.slice(0, 40).map((person, index) => {
...
async function trainingNeeds(tenantId) {
...
async function improvementTracking(tenantId) {
...
function appraisalTemplate(role) {
    if (['Captain', 'Lieutenant', 'Battalion Chief'].includes(role))
        return 'Company Officer Template';
    if (role.includes('Officer') || role.includes('Chief'))
        return 'Staff Officer Template';
...
function peerFeedbackFor(personnel, index) {
...
async function appraisals(tenantId) {
    const ctx = await getContext(tenantId);
    return ctx.personnel.slice(0, 24).map((person, index) => {
...
async function appraisalCycleSummary(tenantId) {
...
async function escalations(tenantId) {
...
async function workforceForecast(tenantId, days = 7) {
    const staffing = await staffingService_js_1.staffingService.getForecast(tenantId, days);
    const board = (await staffingService_js_1.staffingService.getBoard(tenantId, 1, 100)).items;
    const ctx = await getContext(tenantId);
...
async function requisitions(tenantId) {
    const board = (await staffingService_js_1.staffingService.getBoard(tenantId, 1, 100)).items.filter((row) => row.gap > 0);
...
async function writeAudit(tenantId, userId, action, entityName, entityId) {
    await prisma_js_1.prisma.auditLog.create({ data: { tenantId, userId: userId ?? null, action, entityName, entityId, before: null, after: { action, recordedAt: new Date().toISOString() }, createdAt: new Date().toISOString() } });
}
...
async function loadItemStates(tenantId) {
    const rows = await prisma_js_1.prisma.workforceItemState.findMany({ where: { tenantId } });
    const map = new Map();
    for (const row of rows)
        map.set(String(row.itemId), row);
    return map;
}
function nextRequisitionStatus(current) {
    const i = REQUISITION_STATES.indexOf(current);
    return REQUISITION_STATES[Math.min(REQUISITION_STATES.length - 1, (i < 0 ? 0 : i) + 1)];
}
function actionToState(kind, action, prevStatus) {
    const a = action.toLowerCase();
    if (kind === 'requisition') {
        if (a.includes('advance'))
            return nextRequisitionStatus(prevStatus ?? 'Draft');
    ...
    if (a.includes('schedule'))
        return 'Scheduled';
    if (a.includes('acknowled'))
        return 'Acknowledged';
    return 'Approved';
}
async function persistedKpis(tenantId) {
    const rows = await prisma_js_1.prisma.workforceKpi.findMany({ where: { tenantId } });
...
async function persistedRequisitions(tenantId) {
    const rows = await prisma_js_1.prisma.workforceStaffRequisition.findMany({ where: { tenantId } });
    return rows.map((r) => ({ ...r, custom: true }));
}
function applyState(rows, kind, states, field = 'status') {
    return rows.map((row) => {
...
exports.workforceService = {
    ...
    async getKpis(tenantId) { return { items: [...await persistedKpis(tenantId), ...kpiLibrary()], categories: kpiCategorySummary() }; },
    async getKpiCategories() { return kpiCategorySummary(); },
    async getDepartmentScorecard() { return departmentScorecard(); },
    async getStationScorecards(tenantId) { return { items: await stationScorecards(tenantId) }; },
    async getPlatoonScorecards(tenantId) { return { items: await platoonScorecards(tenantId) }; },
    async getPersonnelScorecards(tenantId) { return { items: await personnelScorecards(tenantId) }; },
    async getTrainingNeeds(tenantId) { return { items: applyState(await trainingNeeds(tenantId), 'need', await loadItemStates(tenantId)) }; },
    async getImprovement(tenantId) { return { items: await improvementTracking(tenantId) }; },
    async getAppraisals(tenantId) { return { items: applyState(await appraisals(tenantId), 'appraisal', await loadItemStates(tenantId)), summary: await appraisalCycleSummary(tenantId) }; },
    async getEscalations(tenantId) { return { items: applyState(await escalations(tenantId), 'escalation', await loadItemStates(tenantId), 'decisionState') }; },
    async getForecast(tenantId, days = 7) { return workforceForecast(tenantId, days); },
    async getRequisitions(tenantId) {
    ...
    async createRequisition(tenantId, userId, payload) {
        ...
        const saved = await prisma_js_1.prisma.workforceStaffRequisition.create({ data: { ...payload, id, tenantId, status: payload.status ?? 'Draft', linkedForecast: payload.linkedForecast ?? 'Minimum staffing gap forecast', submittedBy: payload.submittedBy ?? 'Battalion Chief (on duty)' } });
    ...
    async actOnItem(tenantId, userId, kind, id, action) {
        ...
        await prisma_js_1.prisma.workforceItemState.create({ data: { id: `state-${kind}-${id}`, tenantId, kind, itemId: id, action, state, at: new Date().toISOString() } });
