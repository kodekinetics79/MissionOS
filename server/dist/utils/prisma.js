...
`);
const rowCount = db.prepare('SELECT COUNT(*) as count FROM records').get();
if ((rowCount?.count ?? 0) === 0) {
    ...
    for (const [model, rows] of Object.entries(seed_js_1.seedData)) {
        for (const row of rows) {
            const id = row.id ?? `${model}-${(0, node_crypto_1.randomUUID)()}`;
            insert.run(model, String(id), row.tenantId ?? null, JSON.stringify({ ...row, id }), row.createdAt ?? null, row.updatedAt ?? null);
        }
    }
}
function loadRows(model) {
    const query = db.prepare('SELECT data FROM records WHERE model = ?');
    return query.all(model).map(({ data }) => JSON.parse(data));
}
function upsertRow(model, row) {
...
function deleteRows(model, predicate) {
...
function pickPathValue(row, key) {
...
function matchCondition(value, condition) {
    if (condition && typeof condition === 'object' && !Array.isArray(condition)) {
        const normalizeDate = (input) => (input instanceof Date ? input.getTime() : Number.isFinite(Date.parse(String(input))) ? new Date(input).getTime() : input);
        if ('in' in condition)
        ...
        if ('lt' in condition)
            return normalizeDate(value) < normalizeDate(condition.lt);
        if ('lte' in condition)
            return normalizeDate(value) <= normalizeDate(condition.lte);
        if ('gt' in condition)
            return normalizeDate(value) > normalizeDate(condition.gt);
        if ('gte' in condition)
            return normalizeDate(value) >= normalizeDate(condition.gte);
        if ('equals' in condition)
            return value === condition.equals;
        if ('is' in condition)
            return matchWhere(value ?? {}, condition.is);
    }
    return value === condition;
}
function relatedRows(model, relation, row) {
...
function paginate(rows, take, skip) {
    const start = skip ?? 0;
...
function withIncludes(model, row, include) {
    ...
    if (model === 'user') {
        if (include.tenant)
            enriched.tenant = loadRows('tenant').find((tenant) => tenant.id === row.tenantId) ?? null;
        if (include.roles) {
    ...
    if (model === 'personnel') {
        if (include.station)
            enriched.station = loadRows('station').find((station) => station.id === row.stationId || station.id === row.currentStationId) ?? null;
        if (include.rankRef)
            enriched.rankRef = loadRows('rank').find((rank) => rank.id === row.rankId) ?? null;
        ...
        if (include.assignments)
            enriched.assignments = loadRows('personnelAssignment').filter((assignment) => assignment.personnelId === row.id);
        if (include.assignmentHistory)
            enriched.assignmentHistory = loadRows('personnelAssignmentHistory').filter((assignment) => assignment.personnelId === row.id);
        if (include.reviews)
            enriched.reviews = loadRows('personnelPerformanceReview').filter((review) => review.personnelId === row.id);
        if (include.goals)
            enriched.goals = loadRows('personnelGoal').filter((goal) => goal.personnelId === row.id);
        if (include.notes)
            enriched.notes = loadRows('personnelNote').filter((note) => note.personnelId === row.id);
        if (include.documents) {
            enriched.documents = loadRows('personnelDocument').filter((document) => document.personnelId === row.id);
        }
        if (include.readinessSnapshots)
            enriched.readinessSnapshots = loadRows('personnelReadinessSnapshot').filter((snapshot) => snapshot.personnelId === row.id);
    }
    if (model === 'station') {
        if (include.personnel)
            enriched.personnel = loadRows('personnel').filter((person) => person.currentStationId === row.id || person.stationId === row.id);
        if (include.apparatus)
            enriched.apparatus = loadRows('apparatus').filter((apparatus) => apparatus.stationId === row.id);
        if (include.shiftAssignments)
            enriched.shiftAssignments = loadRows('shiftAssignment').filter((assignment) => assignment.stationId === row.id);
    }
    if (model === 'apparatus') {
        if (include.station)
            enriched.station = loadRows('station').find((station) => station.id === row.stationId) ?? null;
        if (include.apparatusTypeRef)
            enriched.apparatusTypeRef = loadRows('apparatusType').find((type) => type.id === row.apparatusTypeId) ?? null;
        if (include.maintenanceEvents)
            enriched.maintenanceEvents = loadRows('maintenanceEvent').filter((event) => event.apparatusId === row.id);
        if (include.preventiveSchedules)
            enriched.preventiveSchedules = loadRows('preventiveMaintenanceSchedule').filter((schedule) => schedule.apparatusId === row.id);
    }
    if (model === 'asset') {
        if (include.tenant)
            enriched.tenant = loadRows('tenant').find((tenant) => tenant.id === row.tenantId) ?? null;
        if (include.station)
            enriched.station = loadRows('station').find((station) => station.id === row.stationId) ?? null;
        if (include.apparatus)
            enriched.apparatus = loadRows('apparatus').find((apparatus) => apparatus.id === row.apparatusId) ?? null;
        if (include.assignedPersonnel)
            enriched.assignedPersonnel = loadRows('personnel').find((personnel) => personnel.id === row.assignedPersonnelId) ?? null;
    }
    if (model === 'inventoryItem') {
        ...
        if (include.apparatus)
            enriched.apparatus = loadRows('apparatus').find((apparatus) => apparatus.id === row.apparatusId) ?? null;
        if (include.vendor)
            enriched.vendor = loadRows('vendor').find((vendor) => vendor.id === row.vendorId) ?? null;
    }
    ...
    if (model === 'maintenanceEvent') {
        if (include.apparatus)
            enriched.apparatus = loadRows('apparatus').find((apparatus) => apparatus.id === row.apparatusId) ?? null;
        if (include.asset)
            enriched.asset = loadRows('asset').find((asset) => asset.id === row.assetId) ?? null;
        if (include.vendor)
            enriched.vendor = loadRows('vendor').find((vendor) => vendor.id === row.vendorId) ?? null;
        if (include.reportedByPersonnel)
            enriched.reportedByPersonnel = loadRows('personnel').find((personnel) => personnel.id === row.reportedByPersonnelId) ?? null;
        if (include.assignedToPersonnel)
            enriched.assignedToPersonnel = loadRows('personnel').find((personnel) => personnel.id === row.assignedToPersonnelId) ?? null;
    }
    ...
    if (model === 'personnelCertification') {
        if (include.certification)
            enriched.certification = loadRows('certification').find((certification) => certification.id === row.certificationId) ?? null;
        if (include.personnel)
            enriched.personnel = loadRows('personnel').find((personnel) => personnel.id === row.personnelId) ?? null;
    }
    ...
    if (model === 'trainingAssignment') {
        if (include.personnel)
            enriched.personnel = loadRows('personnel').find((personnel) => personnel.id === row.personnelId) ?? null;
        if (include.course)
            enriched.course = loadRows('course').find((course) => course.id === row.courseId) ?? null;
        if (include.session)
            enriched.session = loadRows('courseSession').find((session) => session.id === row.sessionId) ?? null;
    }
    ...
    if (model === 'incidentPersonnel') {
        if (include.personnel)
            enriched.personnel = loadRows('personnel').find((personnel) => personnel.id === row.personnelId) ?? null;
        if (include.incident)
            enriched.incident = loadRows('incident').find((incident) => incident.id === row.incidentId) ?? null;
    }
    if (model === 'incident') {
        if (include.station)
            enriched.station = loadRows('station').find((station) => station.id === row.stationId) ?? null;
        ...
        if (include.timeline)
            enriched.timeline = loadRows('incidentTimelineEvent').filter((event) => event.incidentId === row.id);
        if (include.narratives)
            enriched.narratives = loadRows('incidentNarrative').filter((entry) => entry.incidentId === row.id);
        if (include.qaReviews)
            enriched.qaReviews = loadRows('incidentQaReview').filter((entry) => entry.incidentId === row.id);
        if (include.attachments)
            enriched.attachments = loadRows('incidentAttachment').filter((entry) => entry.incidentId === row.id);
        if (include.epcrLinks)
            enriched.epcrLinks = loadRows('epcrLink').filter((entry) => entry.incidentId === row.id);
        if (include.nerisExports)
            enriched.nerisExports = loadRows('nerisExportLog').filter((entry) => entry.incidentId === row.id);
        if (include.dataQualityIssues)
            enriched.dataQualityIssues = loadRows('incidentDataQualityIssue').filter((entry) => entry.incidentId === row.id);
        if (include.duplicateCandidates)
            enriched.duplicateCandidates = loadRows('incidentDuplicateCandidate').filter((entry) => entry.incidentId === row.id);
    }
    return enriched;
}
function makeDelegate(model) {
    return {
        ...
        count: async (args = {}) => loadRows(model).filter((row) => matchWhere(row, args.where, model)).length,
        create: async (args = {}) => {
            const row = {
        ...
        update: async (args = {}) => {
            ...
            if (!id)
                throw new Error(`Missing id for ${model}.update`);
            const rows = loadRows(model);
        ...
        deleteMany: async (args = {}) => ({ count: deleteRows(model, (row) => matchWhere(row, args.where, model)) }),
